...
int ipsw_print_info(const char* path)
{
	...
	cprintf("Product Version: %s   Build: %s\n", prod_ver, build_ver);
	free(prod_ver);
	free(build_ver);
	cprintf("Supported Product Types:");
	...
	if (build_ids_group_iter) {
		...
		do {
			...
			cprintf("  [%d] Variant: %s   Behavior: %s\n", group_no, key, rbehavior);
			free(key);
			free(rbehavior);
